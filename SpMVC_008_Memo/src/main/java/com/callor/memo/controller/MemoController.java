...
public class MemoController {
	...
	/*
	 * 첨부파일이 있는 프로젝트에서 form 의 file input box 의 이름은 절대 VO(DTO) 클래스에 선언된 이름을 사용하면
	 * 안된다~~~
	 */
	...
	@RequestMapping(value = "/{seq}/restore", method = RequestMethod.GET)
	public String restore(@PathVariable("seq") String seq) {
		MemoDTO memoDTO = memoService.findById(Long.valueOf(seq));
		memoDTO.setM_status(0);
		memoDao.update(memoDTO);
		return "redirect:/memo/trashcan";
	}
